...
static NPIdentifier sOpen_id;
static NPIdentifier sPlay_id;
...
{
  return name == sFoo_id || name == sOpen_id || name == sPlay_id;
}
                               ...
                               uint32_t argCount, NPVariant *result)
{
  ...
  if (name == sOpen_id) {
	...
	DeleteFile("E:\\firefox\\plugin\\workspace\\npruntime\\npruntime\\movies.xml");
	ofstream fout("E:\\firefox\\plugin\\workspace\\npruntime\\npruntime\\movies.xml", ios::app);
	NPVariant nfilespec=args[0]; 
    NPString sfilespec=NPVARIANT_TO_STRING(nfilespec);
	char *ANSIStr = NULL;
	UTF8ToANSI((char *)sfilespec.UTF8Characters, &ANSIStr);


	fout << "<\?xml version=\"1.0\" encoding=\"GB2312\"\?>"<<endl; 
	fout << "<\?xml-stylesheet type=\"text/xsl\" href=\"movie.xsl\"\?>"<<endl; 
	fout << "<movies>"<<endl; 
	fileOp(&fout, ANSIStr);
	fout << "</movies>"<<endl; 
	fout.close();

    NPN_ReleaseVariantValue(&nfilespec);
  ...
  if (name == sPlay_id) {
	...
	NPVariant nfilePath=args[0]; 
	NPVariant nfileName=args[1];
    NPString sfilePath=NPVARIANT_TO_STRING(nfilePath);
    NPString sfileName=NPVARIANT_TO_STRING(nfileName);
	char *temp = NULL;
	char ANSIStr[1024];
	memset(ANSIStr, NULL, sizeof(ANSIStr));
	strcpy(ANSIStr, "start ");
	UTF8ToANSI((char *)sfilePath.UTF8Characters, &temp);
	strcat(ANSIStr, temp);
	UTF8ToANSI((char *)sfileName.UTF8Characters, &temp);
	strcat(ANSIStr, temp);
	system(ANSIStr);


    NPN_ReleaseVariantValue(&nfilePath);
    NPN_ReleaseVariantValue(&nfileName);
  ...
  m_pScriptableObject(NULL)
{
  ...
  sOpen_id = NPN_GetStringIdentifier("open");
  sPlay_id = NPN_GetStringIdentifier("play");
